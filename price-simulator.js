...
//   - Tailwind CSS (CDN)
//   - price-simulator.css
//   - index.html 内の showPage() 関数
//     （お問い合わせページへの遷移に使用）
//
// 読み込み方法（index.html の </body> 直前に追記）:
//   <link rel="stylesheet" href="./price-simulator.css">
//   <script src="./price-simulator.js" defer></script>
// =============================================
...
(function () {
    ...
    const SPOT_ITEMS = {
        ...
        contract:        { name: '雇用契約書・労働条件通知書 作成', small: 10000,  mid: 12000,  large: 15000  },
        agreement36:     { name: '36協定 作成・届出',               small: 15000,  mid: 20000,  large: 25000  },
        si_new:          { name: '社会保険 新規適用手続き',         small: 20000,  mid: 30000,  large: 40000  },
        si_entry:        { name: '入退社手続き代行（スポット）',    small: 5000,   mid: 8000,   large: 10000  },
        si_count:        { name: '算定基礎届・月額変更届',          small: 20000,  mid: 22000,  large: 25000  },
        ...
        subsidy_app:     { name: '助成金申請代行',                  fixed: '着手金30,000円＋成功報酬20%' },
    ...
    window.simCalc = function () {
        ...
        if (optRules?.checked) {
            fixedOptions += 80000;
            breakdown.push({ label: ADV_OPTIONS.rules.name, amount: 80000, unit: '/回' });
        }
        const optSubsidy = document.querySelector('#adv-options input[value="subsidy"]');
        if (optSubsidy?.checked) {
            breakdown.push({ label: ADV_OPTIONS.subsidy.name, amount: null, note: '成功報酬20%' });
        }
        const optAttend = document.querySelector('#adv-options input[value="attendance"]');
        if (optAttend?.checked) {
            fixedOptions += 50000;
            breakdown.push({ label: ADV_OPTIONS.attendance.name, amount: 50000, unit: '/回' });
        }
        const optEval = document.querySelector('#adv-options input[value="evaluation"]');
        if (optEval?.checked) {
            fixedOptions += 150000;
            breakdown.push({ label: ADV_OPTIONS.evaluation.name, amount: 150000, unit: '/回' });
        }
        const optAudit = document.querySelector('#adv-options input[value="audit"]');
        if (optAudit?.checked) {
            fixedOptions += 80000;
            breakdown.push({ label: ADV_OPTIONS.audit.name, amount: 80000, unit: '/回' });
        }
    ...
    /**
     * シミュレーター結果を持ったままお問い合わせページへ遷移する。
     * showPage() は index.html 側で定義されていること。
     * @param {'advisor'|'spot'} type
     */
